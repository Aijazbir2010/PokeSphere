...
export const loader = async ({ request }: {request: Request}) => {
    return requireAccessToken(request, async (user) => {
        try {
            ...
            const updatedUser = await User.findOneAndUpdate({userId: user.userId}, {$addToSet: {savedPokemons: id}}, {new: true})

            return json({ updatedUser, success: true })
        } catch (err) {
            return json({ error: 'Cannot save pokemon ! Server error !' }, { status: 500 })
        }
    })
